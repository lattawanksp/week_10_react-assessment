...
- ทุกครั้งที่ข้อมูลเปลี่ยน react จะลบแล้ววาดใหม่ทั้งหน้า (re-render)ใช้ useEffect แบบเหมือนทำครั้งแรกครั้งเดียว ไม่ต้องทำซ้ำ [] = คำสั่งให้ทำครั้งเดียวพอ ใช้ดึงข้อมูลจาก API ตอนหน้าโหลด
- API → useEffect → setMembers(data) → members.map() → table
  useEffect → ดึงข้อมูลจาก API ตอนหน้าโหลด
  setMembers → เก็บข้อมูลที่ได้ลงใน state
  members.map() → เอา state มาวนลูปแสดงในตาราง
- คุณสามารถใช้ fetch() โดยไม่ใช้ useEffect ได้หรือไม่? ได้ แต่จะทำให้เกิด infinite loop เพราะทุกครั้งที่ดึงข้อมูลมาแล้ว setMembers() จะทำให้ component render ใหม่ แล้วก็ fetch ซ้ำวนไปเรื่อยๆ useEffect กับ [] จึงเป็นตัวควบคุมให้ fetch ทำงานแค่ครั้งเดียว
